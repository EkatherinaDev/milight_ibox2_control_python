...
Этот проект содержит графический интерфейс для Windows в стиле экрана `Colors`
из приложения Mi-Light 3.0: зоны, цветовой круг, белый режим, насыщенность,
яркость, `Night light`, `ON` и `OFF`. По умолчанию приложение управляет зоной
`0` (`all`, все зоны).
...
Откроется окно управления цветом. В нем можно:

* найти iBox2 в локальной сети по UDP-порту `5987`;
* выбрать `All` или зоны `Zone1`-`Zone4`;
* включить или выключить свет кнопками `ON` и `OFF`;
* выбрать цвет на круге;
* включить белый режим через центр круга `W`;
* настроить `Saturation` и `Brightness`;
* включить `Night light`.
...
3. Оставьте `All`, чтобы управлять зоной `0` (`all`, все зоны), или выберите `Zone1`-`Zone4`.
4. Используйте `ON`, `OFF`, цветовой круг, `W`, `Saturation`, `Brightness` и `Night light`.
